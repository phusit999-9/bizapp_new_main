...
class Reports_model extends CI_Model {
	...
	public function show_sales_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$tot_discount_to_all_amt = $q2->discount_to_all_amt;
                
				$tax = $res2->tax;
				$vat_amt2 = $tax +100 ;
				$vat_amt = $tax ;

				$qVat = $this->db->query("select * from db_store where status=1 and id=" . get_current_store_id());
				$resVat = $qVat->row();

				$company_vat_no = $resVat->vat_no;
				$tax = $company_vat_no+100;
			...
			//foreach ($q1->result() as $res1) {
				...
				echo "<td class='text-center'><a title='View Invoice' href='".base_url("sales/invoice/$res1->id")."'>".$res1->sales_code."</a></td>";
				}
				else{
				echo "<td class='text-center'>".$res1->sales_code."</td>";	
				}
			...
			//	echo "<td class='text-right'>".store_number_format(($res1->grand_total /$tax *$company_vat_no))."</td>";	
				echo "<td class='text-center'>".$res1->created_by."</td>";
				echo "</tr>";
				$tot_grand_total+=$res1->grand_total;
				$tot_paid_amount+=$res1->paid_amount;
				$tot_due_amount+=($res1->grand_total-$res1->paid_amount);
                $vat_amt_all +=($res1->grand_total /$tax *$company_vat_no);
			}
			...
			echo "<tr>
					  <td class='text-right text-bold' colspan='$total_columns_count'><b>รวม :</b></td>
	...
	public function show_sales_return_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				if($store_id==get_current_store_id()){
				echo "<td class='text-center'><a title='View Invoice' href='".base_url("sales_return/invoice/$res1->id")."'>".$res1->return_code."</a></td>";
				}
				else{
				echo "<td class='text-center'>".$res1->return_code."</td>";	
				}

				
				echo "<td class='text-center'>".show_date($res1->return_date)."</td>";
				
				echo (!empty($res1->sales_code)) ? "<td class='text-center'><a title='Return Raised Against this Invoice' href='".base_url("sales/invoice/$res1->id")."'>".$res1->sales_code."</a></td>" : '<td>-NA-</td>';
				echo "<td>".$res1->customer_name."</td>";
				echo "<td class='text-right'>".store_number_format($res1->grand_total)."</td>";
				echo "<td class='text-right'>".store_number_format($res1->paid_amount)."</td>";
				echo "<td class='text-right'>".store_number_format(($res1->grand_total-$res1->paid_amount))."</td>";
	...
	public function show_purchase_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				if($store_id==get_current_store_id()){
				echo "<td class='text-center' ><a title='View Invoice' href='".base_url("purchase/invoice/$res1->id")."'>".$res1->purchase_code."</a></td>";
				}
				else{
				echo "<td class='text-center'>".$res1->purchase_code."</td>";	
				}

				
				echo "<td class='text-center'>".show_date($res1->purchase_date)."</td>";
				echo "<td class='text-center'>".$res1->supplier_code."</td>";
	...
	public function show_purchase_return_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				if($store_id==get_current_store_id()){
				echo "<td class='text-center'><a title='View Invoice' href='".base_url("purchase_return/invoice/$res1->id")."'>".$res1->return_code."</a></td>";
				}
				else{
				echo "<td class='text-center'>".$res1->return_code."</td>";	
				}
				...
				echo "<td class='text-center'>".show_date($res1->return_date)."</td>";
				echo (!empty($res1->purchase_code)) ? "<td class='text-center'><a title='Return Raised Against this Invoice' href='".base_url("purchase/invoice/$res1->id")."'>".$res1->purchase_code."</a></td>" : '<td>-NA-</td>';
	...
	public function show_expense_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<td class='text-center'>".$res1->expense_code."</td>";
				echo "<td class='text-center'>".show_date($res1->expense_date)."</td>";
				echo "<td class='text-center'>".$res1->category_name."</td>";
				echo "<td class='text-center'>".$res1->reference_no."</td>";
				echo "<td>".$res1->expense_for."</td>";
				echo "<td class='text-right'>".store_number_format($res1->expense_amt)."</td>";
				echo "<td>".$res1->note."</td>";
				echo "<td class='text-center'>".ucfirst($res1->created_by)."</td>";
				echo "</tr>";
			...
			echo "<tr>
					  <td class='text-right text-bold' colspan='$total_columns_count'><b>ยอดรวมค่าใช้จ่าย :</b></td>
	...
	public function show_stock_report(){
		...
		if($q1->num_rows()>0){
			...
			$str .= "<tr>
					  <td class='text-right text-bold' colspan='$total_columns_count'><b>รวม :</b></td>
					  <td class='text-right text-bold'>".format_qty($tot_stock)."</td>
	...
	public function brand_wise_stock(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
					...
					$str1 .= "<td class='text-center'>".$res1->brand_name."</td>";
					$str1 .= "<td class='text-center'>".format_qty($available_qty)."</td>";
					$str1 .= "</tr>";
			...
			$str1 .= "<tr>
					  <td class='text-right text-bold' colspan='$total_columns_count'><b>รวม :</b></td>
					  <td class='text-center text-bold'>".format_qty($tot_stock)."</td>
				  </tr>";
	...
	public function show_purchase_payments_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<td class='text-center'>".show_date($res1->payment_date)."</td>";
				echo "<td class='text-center'>".$res1->supplier_code."</td>";
				echo "<td>".$res1->supplier_name."</td>";
				echo "<td class='text-center'>".$res1->payment_type."</td>";
	...
	public function show_sales_payments_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<td class='text-center'>".show_date($res1->payment_date)."</td>";
				echo "<td class='text-center'>".$res1->customer_code."</td>";
				echo "<td>".$res1->customer_name."</td>";
				echo "<td class='text-center'>".$res1->payment_type."</td>";
				echo "<td>".$res1->payment_note."</td>";
				echo "<td class='text-right'>".store_number_format(($res1->payment))."</td>";
	...
	public function get_profit_loss_report(){
            ...
            $info['total_purchase_amt']=(store_number_format($pur_total));
			...
			$info['pur_return_total_all']=(store_number_format($pur_return_total));
            ...
            $info['purchase_return_paid_amount']=(store_number_format($purchase_return_paid_amount));
			 ...
			 //Due
			 $purchase_return_due_total= ($pur_return_total+$purchase_return_tax_amt ) - $query->paid_amount;
         ...
         //   $info['sales_tax_amt']=(store_number_format($sales_tax_amt));
			...
			$info['sales_discount_item']=(store_number_format($sales_discount_amt));
			...
			$this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount");
		    $this->db->select("COALESCE(vat, 0) as vat"); // เพิ่มบรรทัดนี้
			...
			// ภาษี vat
            $sales_vat= $query->vat;
         //   $info['sales_tax_amt']=(store_number_format($sales_vat));
            

            //Discount to All
            $sales_discount_all=$query->tot_discount_to_all_amt;
            $info['sales_discount_amt']=(store_number_format($sales_discount_all));
            
            
            //sale total   -ยอดในใบเสร็จ
            $sal_total=$query->tot_sal_grand_total;       
            $info['sal_total']=(store_number_format( $sal_total));

			  //Total SAles amount   - ยอดขายก่อนลดทั้งบิล
			  $total_sale = $sal_total +  $sales_discount_all ;       
			  $info['total_sale']=(store_number_format( $total_sale));
			...
			$dis1 = ($total_sale) / 100 ; //คำนวนราคาขาย 1 เปอร์เซ็นต์
			$disc1 = ($sales_discount_all/ $dis1) ; // คำนวนส่วนลดเป็น เปอร์เซ็นต์
			$dis_vat = $sales_tax_amt / $sales_vat; // คำนวน vat เป็น 1 เปอร์เซ็นต์
		  
			$tax_in = ($dis_vat * 100)+  $sales_tax_amt ;  //ราคาสินค้า+vat                   
															 
			$vat_in = $sales_tax_amt - ($sales_tax_amt/100 * $disc1);   // จำนวนเงิน vat
		

			
			//total sales amt  ภาษี vat
          //  $sales_tax= $sales_tax_amt;
          //  $info['sales_tax_amt']=(store_number_format($sales_tax));
	...
	public function get_profit_by_item(){
		...
		if($q1->num_rows()>0){
			...
			echo "<tr>
					  <td class='text-right text-bold' colspan='2'><b>รวม :</b></td>
	...
	public function get_profit_by_invoice(){
		...
		if($q1->num_rows()>0){
			...
			echo "<tr>
					  <td class='text-right text-bold' colspan='4'><b>รวม :</b></td>
					  <td class='text-right text-bold'>".(store_number_format($tot_sales_cost))."</td>
					  <td class='text-right text-bold'>".(store_number_format($tot_purchase_price))."</td>
...
//		$this->db->group_by("c.`item_id`");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$discount_amt = $q2->discount_amt;
		     	...
		     	$dis1 = ($res1->grand_total + $tot_discount_to_all_amt) / 100 ; //คำนวนราคา 1 เปอร์เซ็นต์

                $disc1 = ($tot_discount_to_all_amt / $dis1) ; // คำนวนส่วนลดเป็น เปอร์เซ็นต์
                $dis_vat =  $tax_amt / $vat_sale; // คำนวน vat เป็น 1 เปอร์เซ็นต์
              
                $tax_in = ($dis_vat * 100)+$tax_amt ;  //ราคาสินค้า+vat                   
                                                                 
                $vat = $tax_amt - ($tax_amt/100 * $disc1);   // จำนวนเงิน vat
				$tax_in_dis = ($vat / $vat_sale) *100;   //ราคาสินค้ารวมVAT  -ลบส่วนลดแล้ว 
				...
				if($res1->store_id==get_current_store_id()){
				echo "<td class='text-center'><a data-toggle='tooltip' target='_blank' title='View Invoice' href='".base_url("sales/invoice/$res1->id")."'>".$res1->sales_code."</a></td>";
				}
				else{
				echo "<td>".$res1->sales_code."</td>";	
				}
				...
				echo "<td class='text-right'>".store_number_format($tot_discount_to_all_amt)."</td>";			
				echo "<td class='text-right'>".store_number_format($vat)."</td>";		
				echo "<td class='text-right'>".store_number_format($res1->grand_total)."</td>";
				echo "</tr>";
			
				$tot_discount_amt +=$tot_discount_to_all_amt;
				$tot_tax_in_amt +=$tax_in_sub;
				$tot_tax_amt +=$vat;
				$tot_grand_total +=$res1->grand_total;
			}
			...
			echo "<tr>
				 ...
				 <!--     <td class='text-right text-bold'>".store_number_format($dis)."</td> -->	 
	...
	public function show_purchase_tax_report(){
		...
		$qVat = $this->db->query("select * from db_store where status=1 and id=" . get_current_store_id());
			...
			//	$vat_amt =($res1->grand_total / $vat_amt2) *$company_vat_no ;
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				if($res1->store_id==get_current_store_id()){
				echo "<td class='text-center'><a data-toggle='tooltip' target='_blank' title='View Invoice' href='".base_url("purchase/invoice/$res1->id")."'>".$res1->purchase_code."</a></td>";
				}
				else{
				echo "<td class='text-center'>".$res1->purchase_code."</td>";	
				}
				...
				echo "<td>".$res1->supplier_name."</td>";
				echo "<td class='text-center'>".$res1->tax_number."</td>";
	...
	public function show_stock_transfer_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<td class='text-center'>".show_date($res1->transfer_date)."</td>";
				echo "<td class='text-center'>".$res1->from_warehouse."</td>";
				echo "<td class='text-center'>".$res1->to_warehouse."</td>";
				echo "<td class='text-center'>".$res1->item_name."</td>";
				echo "<td class='text-center'>".$res1->category_name."</td>";
				echo "<td class='text-center'>".$res1->brand_name."</td>";
				echo "<td class='text-center'>".format_qty($res1->transfer_qty)."</td>";
			...
			echo "<tr>
					  <td class='text-right text-bold' colspan='$total_columns_count'><b>รวม :</b></td>
					  <td class='text-center' text-bold'>".format_qty($tot_transfer_qty)."</td>
				  </tr>";
		}
		else{
			$total_columns_count=9;
