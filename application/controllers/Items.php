...
class Items extends MY_Controller {
	...
	public function newitems(){
		...
		$this->form_validation->set_rules('sales_price', 'Sales Price', 'trim|required');
			...
			if($this->input->post('existing_row_count')==1){
				echo "ไม่มีตัวเลือกสินค้า, กรุณาเลือก!!";exit();
			}
		}		
		if ($this->form_validation->run() == TRUE) {
			...
			echo "กรุณากรอกข้อกำหนด ในช่องที่มีเครื่องหมาย ( * ) .";
		}
	}
	...
	public function ajax_list()
	{
		...
		foreach ($list as $items) {
			...
			$str = "<label class='text-green'>".$items->item_name."</label>";
			...
			$service_or_item_name = ($items->service_bit) ? 'บริการ' : "สินค้า";

			$row[] = $items->category_name."<br>[<label class='text-blue'>".$service_or_item_name."</label>]";
